...
$(document).ready(function(){
	...
	function getList(){
		...
		$.get("message.php",{"do":"list"},function(result){
		   //使用json文件模拟接口数据
		   //$.get("js/list.json",{"do":"list"},function(result){
			console.log(result);
			// 如果有数据，就在页面显示数据(留言)
			if(result.status==1){
				var list = result.data;//留言内容
				var str='';

				for (var k in list) {
					// list[k]={cid: "11", uid: "1", content: "sadf", dateline: "2017-12-08 09:57:01", support: "0", …}
					str+='<dl>';
					str+='<dt>';
					str+='<strong>'+list[k].username+'</strong> 说 :';
					str+='<span style="float:right">'+list[k].dateline+'</span>';
					str+='</dt>';
					str+='<dd>'+list[k].content+'</dd>';
					str+='<dd class="t">';
					str+='<a href="javascript:;" id="support">顶(<span>'+list[k].support+'</span>)</a>';
					str+=' | ';
					str+='<a href="javascript:;" id="oppose">踩(<span>'+list[k].oppose+'</span>)</a>';
					str+='</dd>';
					str+='</dl>';
				}
				//写入到页面
	...
	$("#logout").bind("click",function(){
		$.post("message.php",{"do":"logout"},function(result){
			console.log(result);
			//登录成功，清空用户欢迎信息，显示登录，注册按钮
			if(result.status==1){
				$("#userinfo").empty();
				$("#user").hide();
				$("#reg,#login").show();	
